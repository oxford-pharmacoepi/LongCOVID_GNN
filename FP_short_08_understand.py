...
def run_web_fp_visualizer(sample_size=10000, top_diseases=100, top_drugs=50, max_path_length=4, max_paths_per_pair=50):
    """
...
# def run_enhanced_topology_analysis(confidence_threshold=0.8, sample_size=2000):
#     """
#     Run enhanced topology analysis with more detailed insights
#     """
    
#     print("🔬 ENHANCED TOPOLOGY ANALYSIS")
#     print("="*60)
#     print("🎯 Goal: Understand WHY certain hubs are used while others aren't")
...
#     paths_data = find_web_optimized_paths_filtered(G, fp_pairs_data[:10000], max_path_length=5, max_paths_per_pair=300)
    
#     # Run original topology analysis
#     topology_stats = analyze_topological_patterns(fp_pairs_data, network_data, paths_data, confidence_threshold)
    
#     # NEW: Enhanced analyses
#     print("\n" + "="*60)
#     used_hubs, unused_hubs, type_stats = analyze_hub_selectivity(topology_stats, network_data, paths_data)
    
#     pathway_patterns, drug_disease_patterns = analyze_biological_pathway_patterns(paths_data)
    
#     high_stats, low_stats = compare_high_vs_low_confidence_topology(transformer_fps, paths_data, network_data)
...
#     # Summary insights
#     print("\n🎯 KEY INSIGHTS:")
#     print("1. Hub selectivity shows your GNN uses FUNCTIONAL topology, not structural degree")
#     print("2. Specific genes (NR3C1, PTGS2) are pathway-critical for drug-disease connections")
#     print("3. Generic hubs (therapeutic areas) unused = biological specificity")
#     print("4. This strongly supports your TOPOLOGY HYPOTHESIS!")
    
#     return {
#         'topology_stats': topology_stats,
#         'used_hubs': used_hubs,
#         'unused_hubs': unused_hubs,
...
# # # USAGE: Add this to your script
# # if __name__ == "__main__":
# #     print("🔬 Running Enhanced Topology Analysis...")
# #     results = run_enhanced_topology_analysis(confidence_threshold=0.8, sample_size=2000)
...
# def analyze_gene_shortcuts_to_eye_infection(G, paths_data, network_data, eye_infection_idx):
#     """
#     Analyze if certain genes create 'shortcuts' to eye infection
#     A shortcut = a gene that appears in many different drug->eye_infection paths
#     """
    
#     print("🔍 ANALYZING GENE SHORTCUTS TO EYE INFECTION")
#     print("="*60)
    
#     # Extract all paths that lead to eye infection
#     eye_infection_paths = []
#     eye_infection_drugs = set()
    
#     for pair_key, data in paths_data.items():
#         if "eye infection" in pair_key.lower() and data['paths_found'] > 0:
#             drug_name = data['pair']['drug_name']
#             eye_infection_drugs.add(drug_name)
...
#     print(f"📊 EYE INFECTION PATH ANALYSIS:")
#     print(f"   • Total eye infection paths: {len(eye_infection_paths)}")
#     print(f"   • Unique drugs leading to eye infection: {len(eye_infection_drugs)}")
...
#         # A gene is a "shortcut" if:
#         # 1. It's used by many different drugs (high drug count)
#         # 2. It's used frequently (high total usage)
#         # 3. It appears multiple times per drug (high usage per drug)
        
#         shortcut_score = num_drugs * usage_per_drug * (total_usage / len(eye_infection_paths))
...
#             'drug_coverage': num_drugs / len(eye_infection_drugs) if len(eye_infection_drugs) > 0 else 0
#         })
    
#     # Sort by shortcut score
#     shortcut_genes.sort(key=lambda x: x['shortcut_score'], reverse=True)
    
#     print(f"🚨 TOP SHORTCUT GENES (genes that many drugs use to reach eye infection):")
#     print(f"{'Rank':<4} {'Gene':<20} {'Usage':<6} {'Drugs':<6} {'Coverage':<8} {'Score':<8}")
#     print("-" * 60)
    
#     # COMPLETE FIX: Handle all formatting issues
#     for i, gene_info in enumerate(shortcut_genes[:15], 1):
#         gene_name_short = gene_info['gene'][:19]  # Slice gene name first
#         coverage_pct = f"{gene_info['drug_coverage']:.1%}"  # Format percentage first
        
#         print(f"{i:<4} {gene_name_short:<20} "
#               f"{gene_info['total_usage']:<6} "
#               f"{gene_info['num_drugs']:<6} "
#               f"{coverage_pct:<8} "  # Use pre-formatted percentage
#               f"{gene_info['shortcut_score']:.2f}")
    
#     return shortcut_genes, gene_usage_by_drug

# def analyze_direct_connections_to_eye_infection(G, eye_infection_idx, network_data):
#     """
#     Analyze direct connections to eye infection node
#     """
    
#     print(f"\n🎯 DIRECT CONNECTIONS TO EYE INFECTION")
#     print("="*50)
    
#     if eye_infection_idx is None:
#         print("❌ Eye infection node not found")
#         return None
    
#     # Get direct neighbors of eye infection
#     direct_neighbors = list(G.neighbors(eye_infection_idx))
    
#     print(f"📊 Eye infection has {len(direct_neighbors)} direct neighbors")
...
# def find_common_path_patterns_to_eye_infection(paths_data):
#     """
#     Find common path patterns that lead to eye infection
#     """
    
#     print(f"\n🛤️ COMMON PATH PATTERNS TO EYE INFECTION")
#     print("="*50)
...
# def detect_hub_genes_for_eye_infection(shortcut_genes, G, eye_infection_idx):
#     """
#     Detect if shortcut genes are actually hub genes in the network
#     """
    
#     print(f"\n🌟 HUB GENE ANALYSIS FOR EYE INFECTION SHORTCUTS")
#     print("="*60)
    
#     # Get top shortcut genes
#     top_shortcuts = shortcut_genes[:10]
    
#     print(f"🔍 ANALYZING TOP 10 SHORTCUT GENES:")
#     print(f"{'Gene':<20} {'Shortcut Score':<15} {'Network Degree':<15} {'Distance to Eye':<15}")
#     print("-" * 70)
...
#             # Calculate shortest path to eye infection
#             try:
#                 distance_to_eye = nx.shortest_path_length(G, gene_node, eye_infection_idx)
#             except:
#                 distance_to_eye = "No path"
...
#             # FIXED: Store the sliced gene name in a variable first
#             gene_name_short = gene_name[:19]  # Slice BEFORE formatting
...
#         if isinstance(analysis['distance_to_eye'], int) and analysis['distance_to_eye'] <= 2:
#             if analysis['network_degree'] > 500:  # High degree threshold
#                 network_hubs.append(analysis)
#                 print(f"🌟 HUB: {analysis['gene']} (degree: {analysis['network_degree']}, distance: {analysis['distance_to_eye']})")
#             else:
#                 true_shortcuts.append(analysis)
#                 print(f"🎯 SHORTCUT: {analysis['gene']} (degree: {analysis['network_degree']}, distance: {analysis['distance_to_eye']})")
    
#     print(f"\n📊 SUMMARY:")
#     print(f"   • True shortcuts (low degree, close to eye infection): {len(true_shortcuts)}")
#     print(f"   • Network hubs (high degree, acting as shortcuts): {len(network_hubs)}")
    
#     if len(network_hubs) > len(true_shortcuts):
#         print(f"   🚨 FINDING: Eye infection confidence likely due to NETWORK HUBS!")
#         print(f"   → Model uses high-degree genes as easy paths to eye infection")
#     else:
#         print(f"   🎯 FINDING: Eye infection confidence due to SPECIFIC GENE SHORTCUTS!")
#         print(f"   → Model learned biologically meaningful connections")
    
#     return true_shortcuts, network_hubs


# def create_shortcut_report(shortcut_genes, true_shortcuts, network_hubs, results_path, datetime):
#     """
#     Create a detailed report about gene shortcuts to eye infection
#     """
    
#     print(f"\n📝 CREATING GENE SHORTCUTS REPORT")
#     print("="*40)
    
#     report_filename = f"Gene_Shortcuts_Eye_Infection_Report_{datetime}.txt"
#     report_path = os.path.join(results_path, report_filename)
    
#     with open(report_path, 'w') as f:
#         f.write("GENE SHORTCUTS TO EYE INFECTION ANALYSIS\n")
#         f.write("="*50 + "\n\n")
        
#         f.write("SUMMARY:\n")
#         f.write(f"Total genes analyzed: {len(shortcut_genes)}\n")
#         f.write(f"True shortcuts found: {len(true_shortcuts)}\n")
#         f.write(f"Network hubs acting as shortcuts: {len(network_hubs)}\n\n")
        
#         f.write("TOP 20 SHORTCUT GENES:\n")
#         f.write("-" * 30 + "\n")
#         for i, gene_info in enumerate(shortcut_genes[:20], 1):
#             f.write(f"{i:2d}. {gene_info['gene']:<25} "
#                    f"Score: {gene_info['shortcut_score']:.2f}, "
#                    f"Usage: {gene_info['total_usage']}, "
#                    f"Drugs: {gene_info['num_drugs']}\n")
        
#         f.write(f"\nTRUE SHORTCUTS (biological relevance):\n")
#         f.write("-" * 30 + "\n")
#         for shortcut in true_shortcuts:
#             f.write(f"- {shortcut['gene']}: "
#                    f"degree {shortcut['network_degree']}, "
#                    f"distance {shortcut['distance_to_eye']}\n")
        
#         f.write(f"\nNETWORK HUBS (structural shortcuts):\n")
#         f.write("-" * 30 + "\n")
#         for hub in network_hubs:
#             f.write(f"- {hub['gene']}: "
#                    f"degree {hub['network_degree']}, "
#                    f"distance {hub['distance_to_eye']}\n")
    
#     print(f"✅ Report saved: {report_filename}")
#     return report_path

# # MAIN FUNCTION TO RUN COMPLETE SHORTCUT ANALYSIS
# def run_complete_gene_shortcuts_analysis():
#     """
#     Run complete analysis of gene shortcuts to eye infection
#     """
    
#     print("🧬 COMPREHENSIVE GENE SHORTCUTS ANALYSIS")
#     print("="*70)
#     print("🎯 Goal: Understand if genes create shortcuts to eye infection")
...
#     # Get FP pairs data
#     fp_pairs_data = []
#     for i, (drug_name, disease_name, confidence) in enumerate(transformer_fps[:500]):  # More pairs for better analysis
#         if drug_name in approved_drugs_list_name and disease_name in disease_list_name:
#             drug_idx = approved_drugs_list_name.index(drug_name)
#             disease_list_pos = disease_list_name.index(disease_name)
#             disease_id = disease_list[disease_list_pos]
...
#     # Get paths (you might already have this from previous analysis)
...
#     shortcut_genes, gene_usage_by_drug = analyze_gene_shortcuts_to_eye_infection(G, paths_data, network_data, eye_infection_idx)
    
#     neighbor_types = analyze_direct_connections_to_eye_infection(G, eye_infection_idx, network_data)
    
#     path_patterns, path_examples = find_common_path_patterns_to_eye_infection(paths_data)
    
#     true_shortcuts, network_hubs = detect_hub_genes_for_eye_infection(shortcut_genes, G, eye_infection_idx)
    
#     # Create report
#     report_path = create_shortcut_report(shortcut_genes, true_shortcuts, network_hubs, results_path, datetime)
    
#     print(f"\n🎊 GENE SHORTCUTS ANALYSIS COMPLETE!")
#     print("="*70)
    
#     # Summary insights
#     print(f"\n🎯 KEY FINDINGS:")
#     if len(network_hubs) > len(true_shortcuts):
#         print("1. 🚨 NETWORK HUB BIAS: High-degree genes create shortcuts to eye infection")
#         print("   → Model exploits structural network properties")
#         print("   → High confidence may be due to network topology, not biology")
#     else:
#         print("1. 🎯 BIOLOGICAL SHORTCUTS: Specific genes create meaningful paths")
#         print("   → Model learned genuine biological connections")
#         print("   → High confidence reflects real drug-disease relationships")
...
#     print(f"4. Most common path pattern shows consistent biological mechanism")
...
# # USAGE INSTRUCTIONS
# if __name__ == "__main__":
#     print("🧬 GENE SHORTCUTS ANALYSIS FOR EYE INFECTION")
#     print("="*60)
#     print("This analysis will reveal if certain genes create 'shortcuts' to eye infection")
#     print("and whether your model's high confidence is due to:")
#     print("  🎯 Biological relevance (true shortcuts)")
#     print("  🌟 Network topology bias (hub genes)")
#     print()
    
#     # Run the analysis
#     results = run_complete_gene_shortcuts_analysis()
    
#     if results:
#         print(f"\n📁 Detailed report saved at: {results['report_path']}")
#         print("🔍 Check the report for complete findings!")